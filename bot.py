...
ADD = '' # Your SA-MP Server IP
NUM = '7777' # Your SA-MP Server Port

# SA-MP Server Info CMD
@bot.command()
async def ip(ctx):
...
async def ip_error(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        await ctx.send('Usage: $ip')
        
# SA-MP Ingame Players Info
@bot.command()
...
async def players_error(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        await ctx.send('Usage: $players')


#RCON COMMAND
@bot.command()
async def rcon(ctx,PASS,*cmd):
...
@bot.event
async def on_ready():
    activity = discord.Game(name="IrandaamUlagam.Co", type=3)
